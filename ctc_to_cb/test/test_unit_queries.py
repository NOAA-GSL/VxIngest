...
def test_epoch_fcstlen_model(request):
    ...
    try:
        ...
        _expected_time = 3.0
        _statement = open("./ctc_to_cb/test/test_epoch_fcstLen_model.n1ql", encoding='utf-8').read()
...
def test_epoch_fcstlen_obs(request):
    ...
    try:
        _name = request.node.name
        _expected_time = 0.2
        _statement = open("./ctc_to_cb/test/test_epoch_fcstLen_obs.n1ql", encoding='utf-8').read()
        result = connect_cb()["cluster"].query(_statement, QueryOptions(metrics=True))
        # have to read the rows before we can get to the metadata as of couchbase 4.1
        _rows = list(result.rows())
        elapsed_time = result.metadata().metrics().elapsed_time().total_seconds()
...
def test_forecast_valid_epoch(request):
    ...
    try:
        _name = request.node.name
        _expected_time = 4.0
        _statement = open("./ctc_to_cb/test/test_forecast_valid_epoch.n1ql", encoding='utf-8').read()
        result = connect_cb()["cluster"].query(_statement, QueryOptions(metrics=True))
        # have to read the rows before we can get to the metadata as of couchbase 4.1
        _rows = list(result.rows())
        elapsed_time = result.metadata().metrics().elapsed_time().total_seconds()
...
def test_get_region_lat_lon(request):
    ...
    try:
        _name = request.node.name
        _expected_time = 0.01
        _statement = open("./ctc_to_cb/test/test_get_region_lat_lon.n1ql", encoding='utf-8').read()
        result = connect_cb()["cluster"].query(_statement, QueryOptions(metrics=True))
        # have to read the rows before we can get to the metadata as of couchbase 4.1
        _rows = list(result.rows())
        elapsed_time = result.metadata().metrics().elapsed_time().total_seconds()
...
def test_get_stations(request):
    ...
    try:
        _name = request.node.name
        _expected_time = 1
        _statement = open("./ctc_to_cb/test/test_get_stations.n1ql", encoding='utf-8').read()
        result = connect_cb()["cluster"].query(_statement, QueryOptions(metrics=True))
        # have to read the rows before we can get to the metadata as of couchbase 4.1
        _rows = list(result.rows())
        elapsed_time = result.metadata().metrics().elapsed_time().total_seconds()
...
def test_get_threshold_descriptions(request):
    ...
    try:
        _name = request.node.name
        _expected_time = 0.01
        _statement = open("./ctc_to_cb/test/test_get_threshold_descriptions.n1ql", encoding='utf-8').read()
        result = connect_cb()["cluster"].query(_statement, QueryOptions(metrics=True))
        # have to read the rows before we can get to the metadata as of couchbase 4.1
        _rows = list(result.rows())
        elapsed_time = result.metadata().metrics().elapsed_time().total_seconds()
