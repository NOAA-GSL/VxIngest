...
class VxIngestManager(Process):
        ...
        try:
            ...
            while True:
                try:
                    ...
                    logging.info(
                        self.threadName + ': IngestManager - processing ' + ingest_document_id)
                    self.process_document_id(ingest_document_id)
                    logging.info(
                        self.threadName + ': IngestManager - finished processing ' + ingest_document_id)
    ...
    def process_document_id(self, ingest_document_id):
        ...
        try:
            logging.info(
                "process_document_id - : start time: " + str(start_process_time))
            ...
            document_map = {}
            logging.info("IngestManager.process_document_id: "
    ...
    def write_document_to_cb(self, file_name, document_map):
        ...
        # noinspection PyBroadException
        try:
            logging.info(self.threadName + ': process_document_id writing documents for '
            ...
            logging.info(
                "process_document_id - executing upsert: stop time: " + str(upsert_start_time))
            if not document_map:
                logging.info(
                    self.threadName + ": process_document_id: would upsert documents but DOCUMENT_MAP IS "
                                      "EMPTY")
            else:
                try:
            ...
            logging.info(
                "process_document_id - executing upsert: stop time: " + str(upsert_stop_time))
            logging.info("process_document_id - executing upsert: elapsed time: " +
                         str(upsert_stop_time - upsert_start_time))
        except Exception as e:
            logging.error(self.threadName + ": *** %s Error writing to Couchbase: in "
                                            "process_document_id writing document ***" + str(e))
            raise e
    ...
    def write_document_to_files(self, file_name, document_map):
        ...
        # noinspection PyBroadException
        try:
            logging.info(self.threadName + ': process_document_id writing documents into ' + self.output_dir + ' for '
                                           'ingest_document :  ' + str(file_name) + "threadName: " + self.threadName)
            write_start_time = int(time.time())
            logging.info(
                "process_document_id - executing write: start time: " + str(write_start_time))
            if not document_map:
                logging.info(
                    self.threadName + ": process_document_id: would write documents but DOCUMENT_MAP IS EMPTY")
            else:
                ...
                try:
                    ...
                    logging.info(self.threadName + ': process_document_id writing documents into ' + complete_file_name)
                    ...
                    logging.info(
                        "process_document_id - trying write: Got Exception - " + str(e))
            write_stop_time = int(time.time())
            logging.info(
                "process_document_id - executing file write: stop time: " + str(write_stop_time))
            logging.info("process_document_id - executing file write: elapsed time: " +
