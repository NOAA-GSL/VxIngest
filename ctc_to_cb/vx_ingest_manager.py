...
class VxIngestManager(Process):
    ...
    def __init__(self, name, load_spec, ingest_document_id_queue, output_dir):
        ...
        Args:
            ...
            ingest_document_id_queue (Object): the ingest document Queue
    ...
    def run(self):
        ...
        infinite loop that only terminates when the ingest_document_queue is
        empty. For each enqueued ingest document it calls
        process_document_id with the ingest_document_id and the couchbase
        connection to process the document.
        """
    ...
    def connect_cb(self):
        """create couchbase cluster and collection objects using the established credentials"""
        logging.info(
        ...
        try:
            ...
            logging.error("*** %s in connect_cb ***%s", self.thread_name, str(_e))
            sys.exit("*** Error when connecting to mysql database: ")

    def process_document_id(self, ingest_document_id):
        """process this ingest document
        Args:
            ingest_document_id (string): This is the ingest document id
        Raises:
            Exception: Exception('ingest_document is undefined')
            _e: exception
        """
        ...
        try:
            logging.info(
                "process_document_id - : start time: %s", str(start_process_time)
            )
            ...
            if self.ingest_type_builder_name in self.builder_map.keys():
                ...
                builder_class = getattr(ctc_builder, self.ingest_type_builder_name)
            ...
            document_map = {}
            logging.info(
                "IngestManager.process_document_id: elapsed time: %s",
    ...
    def write_document_to_cb(self, document_map):
        ...
        try:
            logging.info(
                ": process_document_id writing documents for ingest_document : with threadName: %s",
                self.thread_name,
            )
            ...
            logging.info(
                "process_document_id - executing upsert: stop time: %s",
                str(upsert_start_time),
            )
            if not document_map:
                logging.info(
                    "%s: process_document_id: would upsert documents but DOCUMENT_MAP IS EMPTY",
                    self.thread_name,
                )
            else:
                try:
            ...
            logging.info(
                "process_document_id - executing upsert: elapsed time: %s",
                str(upsert_stop_time - upsert_start_time),
            )
        except Exception as _e:
            logging.error(
                "%s: *** %s Error writing to Couchbase: in process_document_id writing document ***",
                self.thread_name,
                str(_e),
            )
            raise _e
    ...
    def write_document_to_files(self, file_name, document_map):
        ...
        try:
            logging.info(
                "process_document_id writing documents into %s for ingest_documents threadName:  %s",
                self.output_dir,
                self.thread_name,
            )
            write_start_time = int(time.time())
            logging.info(
                "process_document_id - executing write: start time: %s",
                str(write_start_time),
            )
            if not document_map:
                logging.info(
                    "%s: process_document_id: would write documents but DOCUMENT_MAP IS EMPTY",
                    self.thread_name,
                )
            else:
            ...
            logging.info(
                "process_document_id - executing file write: stop time: %s",
                str(write_stop_time),
            )
            logging.info(
                "process_document_id - executing file write: elapsed time: %s",
                str(write_stop_time - write_start_time),
            )
        except Exception as _e:
            logging.error(
                ": *** %s Error writing to files: %s in process_document_id writing document ***",
                self.thread_name,
                str(_e),
            )
            raise _e
