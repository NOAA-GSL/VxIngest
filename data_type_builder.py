...
class DataTypeBuilder(ABC):
    ...
    def convert_fcst_valid_beg_str(self, fcst_VSDB_timestamp):
        _valid_begin_local = dt.datetime.strptime(fcst_VSDB_timestamp, CN.TS_VSDB_FORMAT)
        _valid_begin_epoch = int(calendar.timegm(_valid_begin_local.utctimetuple()))
        _valid_begin_str = dt.datetime.utcfromtimestamp(_valid_begin_epoch).strftime(CN.TS_OUT_FORMAT)
        return _valid_begin_str

    def get_timestamp_and_epoch(self, fcst_iso_timestamp, fcst_lead_hours):
        # convert string to utc epoch
        _valid_begin_local = dt.datetime.strptime(fcst_iso_timestamp, CN.TS_OUT_FORMAT)
        _valid_begin_epoch = int(calendar.timegm(_valid_begin_local.utctimetuple()))
        _valid_begin_str = dt.datetime.utcfromtimestamp(_valid_begin_epoch).strftime(CN.TS_OUT_FORMAT)
        _init_begin_epoch = _valid_begin_epoch - int(fcst_lead_hours) * 60 * 60
        _init_begin_str = dt.datetime.utcfromtimestamp(_init_begin_epoch).strftime(CN.TS_OUT_FORMAT)
        return {CN.FCST_INIT_BEG:_init_begin_str, CN.FCST_INIT_EPOCH:_init_begin_epoch, CN.FCST_VALID_BEG:_valid_begin_str, CN.FCST_VALID_EPOCH:_valid_begin_epoch}
    ...
    def get_id_vsdb_v01_l1l2(self, record):
        ...
        id = "DD::" + \
             ...
             record[CN.OBTYPE] + "::" + \
             record[CN.FCST_LEV] + "::" + \
             str(record[CN.FCST_VALID_EPOCH])
        return id
    ...
    def parse_line_to_record_vsdb_v01_l1l2(self, line, database_name):
        ...
        _record[CN.FCST_VALID_EPOCH] = _valid_timestamp_and_epoch[CN.FCST_VALID_EPOCH]
    ...
    def start_new_document_vsdb_v01_l1l2(self, data_type, record, document_map, database_name):
        ...
        try:
            ...
            document_map[data_type][id] = {
                CN.ID: id,
                CN.TYPE: "DataDocument",
                CN.DATATYPE: data_type,
                CN.SUBSET: database_name,
                CN.DATAFILE_ID: "DF_id",  # placeholder this is To Be Determined!   TODO!!!
                CN.DATASOURCE_ID: "DS_id",  # placeholder this is To Be Determined!   TODO!!!
                CN.VERSION: record[CN.VERSION] if CN.VERSION in keys else None,
                CN.MODEL: record[CN.MODEL] if CN.MODEL in keys else None,
                CN.GEOLOCATION_ID: record[CN.VX_MASK] if CN.VX_MASK in keys else None,
...
class VSDB_V01_SL1L2_builder(DataTypeBuilder):
    ...
    def handle_line(self, data_type, line, document_map, database_name):
        try:
            ...
            if not document_map[data_type][id].get(CN.ID):  # document might be uninitialized
                ...
                document_map[data_type][id][CN.DATA].append(self.get_data_record_vsdb_v01_l1l2(record))
