...
def test_build_load_job_doc():
    ...
    try:
        ...
        assert True, ljd['id'].startswith('LJ:METAR:grib2_to_cb.run_ingest_threads:VXIngest')
    except Exception as _e: #pylint:disable=broad-except
...
def test_vxingest_get_file_list():
    ...
    try:
        ...
        assert True, files == ['/tmp/test/f_fred_01','/tmp/test/f_fred_02','/tmp/test/f_fred_04','/tmp/test/f_fred_05','/tmp/test/f_fred_03']
    except Exception as _e: #pylint:disable=broad-except
        assert False, f"test_build_load_job_doc Exception failure: {_e}"
    finally:
        shutil.rmtree("/tmp/test")
