...
class VxIngestManager(Process):
        ...
        try:
            ...
            while True:
                try:
                    ...
                    logging.info(
                        self.thread_name + ': IngestManager - processing file ' + file_name)
                    self.process_file(file_name)
                    logging.info(
                        self.thread_name + ': IngestManager - finished processing file ' + file_name)
    ...
    def write_document_to_cb(self, file_name, document_map):
        ...
        # noinspection PyBroadException
        try:
            logging.info('%s: process_file writing documents for ingest_document :%s  with threadName: %s', self.thread_name, str(file_name), self.thread_name)
    ...
    def write_document_to_files(self, file_name, document_map):
        ...
        # noinspection PyBroadException
        try:
            logging.info('%s: process_file writing documents into %s ingest_document :  ',self.thread_name, self.output_dir)
            ...
            if not document_map:
                ...
                try:
                    ...
                    logging.info('%s: process_file writing documents into %s', self.thread_name, complete_file_name)
            ...
            logging.error(": *** %s Error writing to files: in process_file writing document %s***", self.thread_name, str(e))
            raise e
