...
cbimport json --cluster couchbase://${host} --bucket vxdata --scope-collection-exp _default.METAR --username ${user} --password ${pwd} --format list --generate-key %id% --dataset file://${PWD}/mats_metadata_and_indexes/metadata_files/test_legacy_reject_stations.json
