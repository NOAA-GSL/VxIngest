...
a document_map to the couchbase database.
        ...
        Attributes:
            file_queue - a shared queue of filenames that are
            netcdf files.
...
class NetcdfIngestManager(Process):
    """
    NetcdfIngestManager is a Process Thread that manages an object pool of
    NetcdfBuilders to ingest data from GSD netcdf files into documents that can be
    ...
    in each netcdf file, and a template that defines how to place the netcdf variable values
    ...
    def run(self):
        ...
        try:
            ...
            try:
                ...
                ingest_document_result = self.collection.get(self.ingest_document_id)
                self.ingest_document = ingest_document_result.content
                self.ingest_type_builder_name = _ingest_document['builder_type']
    ...
    def process_file(self, file_name):
        ...
        try:
            if self.ingest_type_builder_name in self.builder_map.keys():
                ...
                builder_class = getattr(netcdf_builder.NetcdfBuilder, self.ingest_type_builder_name)
        ...
        try:
            ...
            logging.info("process_file - executing upsert: stop time: " + str(_upsert_start_time))
            ...
            logging.info("process_file - executing upsert: stop time: " + str(_upsert_stop_time))
