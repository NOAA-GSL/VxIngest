...
recently_added=$(curl -s -u "${user}:${pwd}" http://${host}:8093/query/service  -d "statement=SELECT COUNT(fcstValidEpoch) as recently_added FROM vxdata --scope-collection-exp  WHERE type='DD' AND version='V01' AND subset='${subset}' AND model='${model}' AND docType='CTC' AND subDocType='CEILING' AND fcstValidEpoch > (CLOCK_MILLIS() / 1000) - 3600 * ${hours};" | jq -r '.results | .[] | .recently_added')
