...
curl -s -u "${user}:${pwd}" http://${host}:8093/query/service  -d "statement=select min(METAR.fcstValidEpoch) as min_fcstValidEpoch, max(METAR.fcstValidEpoch) as max_fcstValidEpoch from ${bucket}.${scope}.${collection}  WHERE type='DD' and docType = 'obs' and subset = '${collection}' and version is not missing" | jq -r '.results | .[] | .max_fcstValidEpoch'
