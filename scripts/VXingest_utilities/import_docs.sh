...
do_import() {
  ...
  cat ${file_list} | while read f; do
    echo 'cbimport json --cluster couchbase://${host} --bucket vxdata --scope-collection-exp _default.METAR  --username ${user} --password ${pwd} --format list --generate-key %id% --dataset file:///${f}'
