...
# ***********************************************************************************************************
# analytics-datasets.sh
#
#   Usage: ./analytics-datasets.sh [options]
#
#   This will output all of the available analytics datasets in the cluster
#
#   Options:
#
...
curl \
  ...
  --silent \
  --data-urlencode "statement=SELECT \`Dataset\`.DataverseName, \`Dataset\`.DatasetName, \`Dataset\`.BucketName FROM \`Metadata\`.\`Dataset\` WHERE DataverseName <> 'Metadata' ORDER BY DataverseName, DatasetName;" \
  "$PROTOCOL://$CLUSTER:$PORT/analytics/service" | \
  jq -r '.results[] | "Dataverse: " + .DataverseName + ", Dataset: " + .DatasetName + ", Bucket:" + .BucketName'
