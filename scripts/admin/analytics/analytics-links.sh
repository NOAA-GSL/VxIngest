...
# analytics-links.sh
#
#   Usage: ./analytics-links.sh [options]
#
#   This will output all of the available analytics links in the cluster
#
#   Options:
#
...
curl \
  ...
  --silent \
  --data-urlencode "statement=SELECT DataverseName, Name AS \`Link\` FROM \`Metadata\`.\`Link\`;" \
  "$PROTOCOL://$CLUSTER:$PORT/analytics/service" | \
  jq -r '.results[] | "Dataverse: " + .DataverseName + ", Link: " + .Link'
