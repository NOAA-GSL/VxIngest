...
_usage() {
 ...
 This will iterate over a directory of *.n1ql files and prepare them, using the name
 of the file as the name of the prepared
 ...
 Options:
  ...
  -d, --directory         (optional) A directory of *.n1ql files containing queries to prepare (default: pwd)
  -f, --find              (optional) A simple string to find in the file
  -w, --replace           (optional) The string to replace found matches with
  -x, --remove            (optional) Whether or not the prepared name should be removed prior to attempting to create (default: false)
...
QUERY_DIR=${QUERY_DIR:=$(pwd)}
FIND_PREPARED=${FIND:=''}
REPLACE_PREPARED=${REPLACE:=''}
REMOVE_PREPARED=${REMOVE_PREPARED:=false}
...
_options() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      -d|--dir) QUERY_DIR=${2} && shift 2;;
      -f|--find) FIND_PREPARED=${2} && shift 2;;
      -w|--replace) REPLACE_PREPARED=${2} && shift 2;;
      -x|--remove) REMOVE_PREPARED=${2} && shift 2;;
...
_validate() {
  ...
  if ! [ -d "$QUERY_DIR" ]; then
    warning "The -d/--dir argument contains a path to an invalid directory" && valid=false
  fi
  # if there are errors
  if ( ! $valid ); then
    exit 1
...
main() {
  ...
  arguments:
    ...
    query_dir: $QUERY_DIR
    find: $FIND_PREPARED
    replace: $REPLACE_PREPARED
    remove: $REMOVE_PREPARED"
  ...
  # loop over each of the found files
  for filename in "$QUERY_DIR/"*.n1ql; do
    # get the name of the file w/o the extension as this will be used for the prepared name
    ...
    output="Preparing: $name"
    echo -en "\r\033[K$output"

    # get the contents of the file
    local statement
    statement=$(cat "$filename")

    # check whether or not we should attempt to delete the prepare prior to creating it
    if ( $REMOVE_PREPARED ); then
      executeN1ql "${query_nodes[$((RANDOM % ${#query_nodes[@]}))]}" "DELETE FROM system:prepareds WHERE name='$name'" > /dev/null
    fi

    # create the prepared
    executeN1ql "${query_nodes[$((RANDOM % ${#query_nodes[@]}))]}" "PREPARE $name FROM $statement" > /dev/null
  done
  echo -en "\r\033[K"
  echo ""
exit
