...
def test_build_load_job_doc():
    ...
    try:
        ...
        assert ljd["id"].startswith(
            "LJ:V01:vxingest.ctc_to_cb.run_ingest_threads:VXIngest:"
        ), (
            f"load_job ID is wrong: {ljd['id']} does not start with 'LJ:V01:vxingest.ctc_to_cb.run_ingest_threads:VXIngest:'"
        )
    except Exception as _e:
