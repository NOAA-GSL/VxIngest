...
_MESSAGES: list[tuple[str, int, str]] = [
...
@pytest.fixture
def synthetic_grib2(tmp_path: Path) -> Path:
    """Generate a multi-message GRIB2 file with all level types
    that VxIngest reads in ``build_document()``.

    Returns the path to the generated file.
    """
    grib_path = tmp_path / "synthetic.grib2"
    with grib_path.open("wb") as fh:
        for type_of_level, level, short_name in _MESSAGES:
            _write_grib2_message(fh, type_of_level, level, short_name)
    return grib_path
