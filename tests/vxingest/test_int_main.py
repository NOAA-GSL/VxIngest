...
def test_one_thread_specify_file_pattern_grib2_job_spec_rt(tmp_path: Path):
    # Save original sys.argv
    original_argv = sys.argv.copy()
    job_id = "JS:METAR:MODEL:GRIB2:HRRR_OPS-TEST:schedule:job:V01"
    # need these args
    sys.argv = [
        "run_ingest",
        "-j",
...
def check_partial_sums(vx_ingest, derived_data):  # @UnusedVariable
...
def check_ctc(vx_ingest, derived_data):  # @UnusedVariable
    for item in derived_data:
        if "DF" in item["id"]:
            continue
        if "LJ" in item["id"]:
