...
oer_str = "%s" % oer[:, -1]
qc_str = "%s" % qcf[:, -1]
assert obs_str == "[19.0 -- -- -- -11.200000000000001 5.2 -- -- -- 3.0 -- -- --]"
assert oer_str == "[-- -- -- -- 2.1 -- --]"
assert qc_str == "[2.0 -- -- -- 2.0 -- -- --]"
bufr.close()
...
np.testing.assert_almost_equal(bend * 1.0e6, 4.8)
bufr.close()
...
str2 = "station_id=%s, lon=%7.2f, lat=%7.2f, time=%6.3f, station_type=%s, levels=%s" % (
    station_id.rstrip(),
    hdr[1],
    hdr[2],
    hdr[3],
